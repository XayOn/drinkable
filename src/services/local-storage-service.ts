...
export class LocalStorageService {
    ...
    public async initialize(): Promise<void> {
        ...
        const configuredTaps = await this.getFromLocalStorage(StorageKey.ConfiguredTaps);
        this._configuredTaps = configuredTaps !== null ? configuredTaps : {};

        const cocktailMakerSettings = await this.getFromLocalStorage(StorageKey.CocktailMakerSettings);
        this._cocktailMakerSettings =
            cocktailMakerSettings !== null ? cocktailMakerSettings : { apiUrl: '', apiKey: '', apiToken: '' };
    ...
    public async setCocktailMakerSettings(settings: { [key: string]: string }) {
        ...
        await this.updateKey(StorageKey.CocktailMakerSettings, JSON.stringify(settings));
    }
    ...
    public getTaps() {
        ...
        this._savedIngredientIds.forEach(id => {
            result[id] = this._configuredTaps[id] !== undefined ? this._configuredTaps[id] : 0;
        });
    ...
    public async updateConfiguredTaps(val: { [key: number]: number }) {
        console.log('Saving taps in localstorage to ', val);
        await this.updateKey(StorageKey.ConfiguredTaps, JSON.stringify(val));
        this._configuredTaps = val;
    }
