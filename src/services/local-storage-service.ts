...
export class LocalStorageService {
  ...
  public async initialize(): Promise<void> {
    const savedIngredients = await this.getFromLocalStorage(
      StorageKey.SavedIngredients,
    );
    this._savedIngredientIds = savedIngredients !== null
      ? savedIngredients.map(String)
      : [];

    const messuarementSystem = await this.getFromLocalStorage(
      StorageKey.MessuarementSystem,
      false,
    );
    this._messuarementSystem = messuarementSystem !== null
      ? messuarementSystem
      : MessuarementSystem.Imperial;
    ...
    const cocktailInformation = await this.getFromLocalStorage(
      StorageKey.CocktailInformation,
    );
    this._cocktailInformation = cocktailInformation !== null
      ? cocktailInformation
      : [];
    ...
    const shoppingLists = await this.getFromLocalStorage(
      StorageKey.ShoppingLists,
    );
    this._shoppingLists = shoppingLists !== null ? shoppingLists : [];

    const configuredTaps = await this.getFromLocalStorage(
      StorageKey.ConfiguredTaps,
    );
    this._configuredTaps = configuredTaps !== null ? configuredTaps : {};

    const cocktailMakerSettings = await this.getFromLocalStorage(
      StorageKey.CocktailMakerSettings,
    );
    this._cocktailMakerSettings = cocktailMakerSettings !== null
      ? cocktailMakerSettings
  ...
  private async migrateFavoriteCocktails() {
    ...
    if (keyExists) {
      const favoriteResponse = await this.getFromLocalStorage(
        StorageKey.FavoriteCocktails,
      );
      const favoriteCocktails = favoriteResponse !== null
        ? favoriteResponse.map(String)
        : [];

      favoriteCocktails.forEach((element: string) => {
        const cocktailInformation = this._cocktailInformation.find((x) =>
          x.id === element
        );
  ...
  public async updateCocktailInformation(
    cocktailInformation: CocktailInformation[],
  ) {
    await this.updateKey(
      StorageKey.CocktailInformation,
      JSON.stringify(cocktailInformation),
    );
  ...
  public getCocktailMakerSettings(){
    return this._cocktailMakerSettings;
  }

  public async setCocktailMakerSettings(settings: { [key: string]: string }) {
    this._cocktailMakerSettings = settings;
    await this.updateKey(
      StorageKey.CocktailMakerSettings,
  ...
  public getTaps() {
    ...
    this._savedIngredientIds.forEach((id) => {
      result[id] = this._configuredTaps[id] !== undefined
        ? this._configuredTaps[id]
        : 0;
    });
  ...
  public getTapForItem(item: Ingredient) {
    console.log(this._savedIngredientIds);
    return this._savedIngredientIds[item.id] || 0;
  }

  public async updateConfiguredTaps(val) {
    await this.updateKey(StorageKey.ConfiguredTaps, JSON.stringify(val));
  }
...
export enum StorageKey {
  CocktailMakerSettings = "cocktail-maker-settings",
  ConfiguredTaps = "configured-taps",
